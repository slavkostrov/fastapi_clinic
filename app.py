...
def _get_dog_by_pk(pk: int) -> Dog | None:
    ...
    if dog is None:
        raise HTTPException(status_code=409, detail=f"Specified PK ({pk}) doesn't exist.")
    return dog
...
def create_dog(dog: Dog) -> Dog:
    ...
    if dog.pk in dogs_db:
        raise HTTPException(status_code=409, detail=f"Specified PK ({dog.pk}) already exists.")
    dogs_db[dog.pk] = dog
    return dog
...
def update_dog(pk: int, dog: Dog) -> Dog:
    ...
    """
    if pk != dog.pk:
        raise HTTPException(status_code=409, detail="Input PK isn't equal to Dog object `pk` attribute.")
